...
export function RegisterPage(){
  ...
  async function submit(event:FormEvent){
    ...
    setError('');if(password.length<12||password.length>128){setError(c.short);return}if(password!==confirm){setError(c.mismatch);return}
    setLoading(true)
    try{
      const response=await fetch('/api/register',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:email.trim(),password,privacyVersion:LEGAL_VERSIONS.privacy,termsVersion:LEGAL_VERSIONS.terms,privacyAcknowledged:true,termsAccepted:true,locale:lang,website:''})})
      if(!response.ok)throw new Error('REGISTRATION_FAILED')
      setStep('done')
    }catch{setError(c.generic)}finally{setLoading(false)}
  }
      ...
      <div className="grid lg:grid-cols-[280px_minmax(0,1fr)]">
        ...
        <section className="min-w-0 p-5 sm:p-8 lg:p-10">
          ...
          {step==='account'&&<div className="mx-auto max-w-[480px] py-6 sm:py-10"><span className="text-[11px] font-semibold uppercase tracking-[.12em] text-[var(--fp-green)]">Step 3 / 3</span><h2 className="mt-2 text-[34px] font-semibold tracking-[-.05em]">{c.account}</h2><p className="mt-3 text-[13px] leading-6 text-[var(--fp-muted)]">{lang==='ru'?'Юридические документы подтверждены. Теперь создайте данные для входа.':'Legal review is complete. Create your sign-in credentials.'}</p>
            <form onSubmit={submit} className="mt-7 space-y-4"><label className="block space-y-2 text-[13px] font-medium text-[var(--fp-muted)]"><span>{c.email}</span><Input autoFocus required type="email" autoComplete="email" maxLength={320} value={email} onChange={e=>setEmail(e.target.value)} className="h-[50px]"/></label><label className="block space-y-2 text-[13px] font-medium text-[var(--fp-muted)]"><span>{c.password}</span><div className="relative"><Input required type={show?'text':'password'} autoComplete="new-password" minLength={12} maxLength={128} value={password} onChange={e=>setPassword(e.target.value)} className="h-[50px] pr-12"/><button type="button" onClick={()=>setShow(v=>!v)} className="absolute inset-y-0 right-0 grid w-11 place-items-center text-[var(--fp-subtle)]">{show?<EyeOff size={16}/>:<Eye size={16}/>}</button></div></label><label className="block space-y-2 text-[13px] font-medium text-[var(--fp-muted)]"><span>{c.confirm}</span><Input required type={show?'text':'password'} autoComplete="new-password" minLength={12} maxLength={128} value={confirm} onChange={e=>setConfirm(e.target.value)} className="h-[50px]"/></label>
