...
type NavKey='overview'|'operations'|'payments'|'approvals'|'counterparties'|'routes'|'invoices'|'treasury'|'reconciliation'|'analytics'|'reports'|'activity'|'api'|'settings'
...
const groups:{key:'work'|'network'|'intelligence'|'system';items:NavItem[]}[]=[
  ...
  {key:'intelligence',items:[{key:'treasury',path:'/treasury',icon:Landmark},{key:'reconciliation',path:'/reconciliation',icon:RefreshCw},{key:'analytics',path:'/analytics',icon:BarChart3},{key:'reports',path:'/reports',icon:BookOpenText},{key:'activity',path:'/activity',icon:Activity}]},
...
function label(key:NavKey,lang:Language,t:WorkspaceDictionary){
  const ruLabels:Partial<Record<NavKey,string>>={overview:'Главная',operations:'Что требует внимания',payments:'Платежи',approvals:'Согласования',counterparties:'Поставщики',routes:'Сравнить варианты',invoices:'Счета',treasury:'План платежей',reconciliation:'Сверка платежей',analytics:'Аналитика',reports:'Отчёты',activity:'История',api:'API',settings:'Настройки'}
  if(lang==='ru'&&ruLabels[key])return ruLabels[key]!
  const local:Partial<Record<NavKey,Record<Language,string>>>= {operations:{ru:'Операции',en:'Attention',fr:'À traiter',de:'Aufgaben',es:'Atención'},approvals:{ru:'Согласования',en:'Approvals',fr:'Approbations',de:'Freigaben',es:'Aprobaciones'},treasury:{ru:'Обязательства',en:'Payment plan',fr:'Plan de paiements',de:'Zahlungsplan',es:'Plan de pagos'},reconciliation:{ru:'Сверка платежей',en:'Reconciliation',fr:'Rapprochement',de:'Abgleich',es:'Conciliación'},activity:{ru:'Активность',en:'History',fr:'Historique',de:'Verlauf',es:'Historial'}}
  if(local[key])return local[key]![lang]
...
export function WorkspaceShell({children}:{children:React.ReactNode}){
  ...
  const [searchOpen,setSearchOpen]=useState(false);const [guideOpen,setGuideOpen]=useState(false);const [query,setQuery]=useState('');const [activeSearchIndex,setActiveSearchIndex]=useState(0);const [activityOpen,setActivityOpen]=useState(false);const [profileOpen,setProfileOpen]=useState(false);const [mobileOpen,setMobileOpen]=useState(false);const [adminAccess,setAdminAccess]=useState(false)
