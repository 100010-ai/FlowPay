...
export default function InvoicesPage(){
  const ws=useWorkspace();const {lang}=useLanguage();const router=useRouter();const t=workspaceDictionaries[lang];const copy=workspaceCopy[lang].invoice
  const [query,setQuery]=useState('');const [status,setStatus]=useState('');const [importing,setImporting]=useState(false);const [importState,setImportState]=useState<ImportState>(null);const [actionId,setActionId]=useState<string|null>(null);const fileRef=useRef<HTMLInputElement>(null)
  ...
  const actions=<div className="flex flex-wrap gap-2"><input ref={fileRef} type="file" accept=".csv,text/csv" className="hidden" onChange={e=>{const file=e.target.files?.[0];if(file)void importCsv(file)}}/><Button variant="secondary" disabled={importing} onClick={()=>fileRef.current?.click()}>{importing?<Loader2 size={14} className="animate-spin"/>:<Upload size={14}/>} {t.common.import}</Button><Button onClick={()=>router.push('/invoices/new')}><Plus size={15}/>{t.invoices.create}</Button></div>
