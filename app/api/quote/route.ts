...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (rules.length === 0) return apiJson({ error: 'NO_ELIGIBLE_PROVIDER_ROUTES', requestId: reqId }, 422, { 'X-Request-ID': reqId })
    ...
    if (safeErrorMessage(error) === 'NO_ELIGIBLE_PROVIDER_ROUTES') return apiJson({ error: 'NO_ELIGIBLE_PROVIDER_ROUTES', requestId: reqId }, 422, { 'X-Request-ID': reqId })
